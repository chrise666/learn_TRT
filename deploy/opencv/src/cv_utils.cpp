...
// 加载单张图片
Mat loadImage(const string &img_path)
{
    // 检查文件是否存在
    ...
    {
        throw runtime_error("Failed to read file: " + img_path);
    }
...
// 加载文件夹内所有图片
vector<ImageData> loadAllImages(const string &folder_path)
{
...
// 图像保存
void saveResults(const std::vector<ImageData> &data, const std::string &output_dir, const std::string &suffix)
{
